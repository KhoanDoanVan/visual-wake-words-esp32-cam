...
namespace {
constexpr char kTag[] = "cam_max_stream";
constexpr char kAccessPointSsid[] = "ESP32-CAM-Max";
constexpr char kAccessPointPassword[] = "visualwake";
constexpr size_t kMaxJpegBytes = 768 * 1024;
constexpr size_t kNetworkChunkBytes = 8 * 1024;
constexpr gpio_num_t kFlashLedGpio = GPIO_NUM_4;

// AI-Thinker ESP32-CAM camera bus. The photographed OV3660 module uses the
// same board connector/pin map as the common OV2640 module.
...
void DisableFlashLed() {
  ...
  config.pin_bit_mask = 1ULL << kFlashLedGpio;
  config.mode = GPIO_MODE_OUTPUT;
  config.pull_down_en = GPIO_PULLDOWN_ENABLE;
  config.intr_type = GPIO_INTR_DISABLE;
  ESP_ERROR_CHECK(gpio_config(&config));
  ESP_ERROR_CHECK(gpio_set_level(kFlashLedGpio, 0));
  gpio_hold_en(kFlashLedGpio);
}
...
camera_config_t MakeCameraConfig() {
  ...
  // This board was previously stable at 10 MHz. Maximum resolution stresses
  // the parallel camera bus, so reliability is more useful than a faster XCLK.
...
bool InitializeCamera() {
  ...
  if (sensor == nullptr) {
    ESP_LOGE(kTag, "Camera sensor handle unavailable");
    return false;
  }
  g_telemetry.sensor_pid = sensor->id.PID;
  if (sensor->id.PID != OV3660_PID) {
    ESP_LOGE(kTag, "QXGA requires OV3660; detected PID=0x%04x", sensor->id.PID);
    return false;
  }
  sensor->set_vflip(sensor, 1);
  sensor->set_brightness(sensor, 1);
  sensor->set_saturation(sensor, -2);
  ESP_LOGI(kTag, "Camera ready: OV3660 PID=0x%04x QXGA=2048x1536 JPEG q=12 XCLK=10MHz",
           sensor->id.PID);
  return true;
}
  ...
  while (true) {
    ...
    if (frame->format == PIXFORMAT_JPEG && frame->len <= kMaxJpegBytes &&
      ...
      if (previous_started_us > 0) {
        const float current_fps = 1000000.0f / (started_us - previous_started_us);
        g_telemetry.fps = g_telemetry.fps == 0.0f
                              ? current_fps
                              : g_telemetry.fps * 0.85f + current_fps * 0.15f;
      }
      g_telemetry.updated_ms = finished_us / 1000;
      g_telemetry.camera_ok = true;
      ++g_telemetry.sequence;
      if (g_telemetry.sequence == 1 || g_telemetry.sequence % 10 == 0) {
        ESP_LOGI(kTag,
                 "FRAME,sequence=%lu,width=%u,height=%u,jpeg_bytes=%u,capture_ms=%.2f,fps=%.3f,psram_free=%u,internal_free=%u",
                 static_cast<unsigned long>(g_telemetry.sequence), g_telemetry.width,
                 g_telemetry.height, static_cast<unsigned>(g_telemetry.jpeg_bytes),
                 g_telemetry.capture_ms, g_telemetry.fps,
                 static_cast<unsigned>(heap_caps_get_free_size(MALLOC_CAP_SPIRAM)),
                 static_cast<unsigned>(heap_caps_get_free_size(MALLOC_CAP_INTERNAL |
                                                               MALLOC_CAP_8BIT)));
      }
      xSemaphoreGive(g_frame_mutex);
    } else if (frame->len > kMaxJpegBytes) {
      ESP_LOGW(kTag, "JPEG exceeds %u-byte diagnostic buffer: %u",
               static_cast<unsigned>(kMaxJpegBytes), static_cast<unsigned>(frame->len));
    }
...
esp_err_t StatusHandler(httpd_req_t* request) {
  ...
  const int length = std::snprintf(
      json, sizeof(json),
      "{\"camera_ok\":%s,\"sensor\":\"OV3660\",\"sensor_pid\":%u,"
      "\"width\":%u,\"height\":%u,\"jpeg_bytes\":%u,\"capture_ms\":%.2f,"
      "\"fps\":%.3f,\"sequence\":%lu,\"updated_ms\":%lld,"
      "\"psram_total\":%u,\"psram_free\":%u,\"psram_largest\":%u,"
      "\"internal_free\":%u,\"internal_largest\":%u,\"flash_led\":false}",
      telemetry.camera_ok ? "true" : "false", telemetry.sensor_pid,
      telemetry.width, telemetry.height, static_cast<unsigned>(telemetry.jpeg_bytes),
      telemetry.capture_ms, telemetry.fps, static_cast<unsigned long>(telemetry.sequence),
...
esp_err_t CaptureHandler(httpd_req_t* request) {
  ...
  std::snprintf(disposition, sizeof(disposition),
                "attachment; filename=esp32_cam_ov3660_qxga_%lu.jpg",
  ...
  while (result == ESP_OK) {
    ...
    for (size_t offset = 0; result == ESP_OK && offset < length;
      ...
      result = httpd_resp_send_chunk(
          request, reinterpret_cast<const char*>(frame + offset),
          std::min(kNetworkChunkBytes, length - offset));
    }
...
bool StartWebServers() {
  ...
  httpd_uri_t root{.uri = "/", .method = HTTP_GET, .handler = RootHandler};
  httpd_uri_t status{.uri = "/status", .method = HTTP_GET, .handler = StatusHandler};
  httpd_uri_t capture{.uri = "/capture", .method = HTTP_GET, .handler = CaptureHandler};
  ...
  httpd_uri_t stream{.uri = "/stream", .method = HTTP_GET, .handler = StreamHandler};
...
extern "C" void app_main(void) {
  DisableFlashLed();
  ESP_LOGI(kTag, "Starting classic ESP32-CAM maximum-resolution stream; flash disabled");
  if (!esp_psram_is_initialized()) {
    ESP_LOGE(kTag, "PSRAM is required for QXGA streaming");
    return;
  }
  ESP_LOGI(kTag, "MEMORY_BOOT,psram_total=%u,psram_free=%u,internal_free=%u",
           static_cast<unsigned>(heap_caps_get_total_size(MALLOC_CAP_SPIRAM)),
           static_cast<unsigned>(heap_caps_get_free_size(MALLOC_CAP_SPIRAM)),
           static_cast<unsigned>(heap_caps_get_free_size(MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT)));
